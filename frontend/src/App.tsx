...
import {
  compareDocuments, getResult, saveNotes,
  ResultResponse, Span, Notes, NoteEntry,
} from './lib/api'
...
export default function App() {
  ...
  // Build changes from BOTH span lists (deduplicated by position)
  const origChanges = result ? buildChanges(result.original_spans) : []
  const revChanges  = result ? buildChanges(result.revised_spans)  : []

  // Merge: use original changes as primary, add rev-only inserts
  const allChanges: Change[] = origChanges.length ? origChanges : revChanges
  ...
  useEffect(() => {
    ...
    saveNotes(result.id, debouncedNotes).catch(() => {})
  ...
  }, [state, allChanges.length])

  const handleCompare = async () => {
    if (!fileA || !fileB) return
    setState('loading'); setProgress(10)
    try {
      const tick = setInterval(() => setProgress(p => Math.min(p+8, 85)), 400)
      const cmp  = await compareDocuments(fileA, fileB)
      clearInterval(tick); setProgress(95)
      const full = await getResult(cmp.id)
      setProgress(100); setResult(full)
      setNotes(full.notes ?? {}); setCurChange(0); setState('result')
    } catch (e: any) {
  ...
  const copyLink = () => {
    if (!result) return
    navigator.clipboard.writeText(`${window.location.origin}/compare/${result.id}`)
    setCopied(true); setTimeout(() => setCopied(false), 2000)
  }

  const reset = () => {
    setFileA(null); setFileB(null); setResult(null)
    setNotes({}); setError(''); setState('upload'); setProgress(0); setZoom(1)
  }

  /* ── Upload / Error ─────────────────────────────────────────────── */
  if (state === 'upload' || state === 'error') return (
    <div className="min-h-screen flex flex-col bg-gray-100">
      ...
      <header className="bg-[#1e3a5f] text-white px-6 py-3.5 flex items-center gap-3 shadow-lg">
        ...
        <span className="text-lg font-bold tracking-tight">DocCompare Pro</span>
        <span className="ml-2 text-[10px] bg-blue-500/30 text-blue-200 px-2 py-0.5 rounded-full font-semibold uppercase tracking-wide">
          Pro
        </span>
      </header>
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-10 w-full max-w-xl">
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">Professional document diff — PDF, DOCX, TXT, XLSX, HTML, MD</p>
            </div>
          </div>

          <div className="flex flex-col gap-4">
            <DropZone label="Original Document" file={fileA} onFile={setFileA} color="blue" />
            <div className="flex items-center gap-3">
              <div className="flex-1 h-px bg-gray-200" />
              <span className="text-xs text-gray-400 font-medium">vs</span>
              <div className="flex-1 h-px bg-gray-200" />
            </div>
            <DropZone label="Revised Document"  file={fileB} onFile={setFileB} color="green" />
          </div>

          {state === 'error' && (
            <div className="mt-5 p-4 bg-red-50 border border-red-200 rounded-xl text-sm text-red-700 flex items-start gap-2">
          ...
          <p className="text-[10px] text-gray-400 text-center mt-5">
            Files deleted after extraction · Results expire in 7 days · Ctrl+←/→ to navigate
          </p>
        </div>
      </main>
    </div>
  ...
  if (state === 'loading') return (
    <div className="min-h-screen flex flex-col bg-gray-100">
      <header className="bg-[#1e3a5f] text-white px-6 py-3.5 flex items-center gap-3 shadow-lg">
        <GitCompare className="w-6 h-6 text-blue-300" />
        <span className="text-lg font-bold">DocCompare Pro</span>
      </header>
  ...
  return (
    <div className="h-screen flex flex-col" style={{ background: '#1a202c' }}>
      ...
      {/* ══ TOOLBAR (Draftable-style dark bar) ══════════════════════════ */}
      <header className="flex items-center gap-1 px-3 py-1.5 bg-[#1e3a5f] shadow-lg flex-shrink-0 border-b border-[#162d4a]">
        ...
        <div className="flex items-center gap-2 pr-3 border-r border-white/10 mr-1">
          <GitCompare className="w-4.5 h-4.5 text-blue-300 w-5 h-5" />
          <span className="text-sm font-bold text-white hidden md:block">DocCompare Pro</span>
        </div>
        ...
        <div className="flex items-center bg-white/10 rounded-md px-1 gap-0.5 mr-1">
          <button className="toolbar-btn px-2 py-1"
            onClick={() => setZoom(z => Math.max(0.6, +(z-0.1).toFixed(1)))} title="Zoom out (Ctrl+-)">
            <ZoomOut className="w-3.5 h-3.5" />
          </button>
          <span className="text-xs text-slate-300 font-mono w-10 text-center select-none">
            {Math.round(zoom*100)}%
          </span>
          <button className="toolbar-btn px-2 py-1"
            onClick={() => setZoom(z => Math.min(1.5, +(z+0.1).toFixed(1)))} title="Zoom in (Ctrl+=)">
        ...
        <div className="flex items-center bg-white/10 rounded-md gap-0.5 mr-1">
          <button className="toolbar-btn px-2 py-1.5" onClick={() => navigate(-1)}
            disabled={curChange === 0} title="Previous change (Ctrl+←)">
          ...
          <button className="toolbar-btn px-2 py-1.5" onClick={() => navigate(1)}
            disabled={curChange >= allChanges.length-1} title="Next change (Ctrl+→)">
        ...
        {/* Change panel toggle */}
        <button className={`toolbar-btn ${showPanel ? 'bg-white/20' : ''}`}
          onClick={() => setShowPanel(v => !v)} title="Toggle change panel">
          <LayoutTemplate className="w-3.5 h-3.5" />
          <span className="hidden md:block">Changes</span>
        </button>

        {/* Share */}
        <button className="toolbar-btn" onClick={copyLink} title="Copy shareable link">
        ...
        {/* Exports */}
        <button className="toolbar-btn" onClick={() => window.open(`/api/export/${result.id}/pdf`, '_blank')}
          title="Download highlighted PDF">
          <Download className="w-3.5 h-3.5" /> <span className="hidden md:block">PDF</span>
        </button>
        <button className="toolbar-btn" onClick={() => window.open(`/api/export/${result.id}/docx`, '_blank')}
          title="Download redline DOCX">
          <FileDown className="w-3.5 h-3.5" /> <span className="hidden md:block">DOCX</span>
        </button>
        <button className="toolbar-btn" onClick={() => window.open(`/api/export/${result.id}/csv`, '_blank')}
          title="Download CSV change report">
      ...
      <div className="flex items-center gap-3 px-4 py-1.5 bg-[#2d3748] border-b border-[#1a202c] text-[11px] flex-shrink-0 flex-wrap">
        ...
        {[
          ...
          ['diff-replace',   'Modified'],
          ['diff-move-from', 'Moved (from)'],
          ['diff-move-to',   'Moved (to)'],
        ].map(([cls, label]) => (
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 min-w-0 flex flex-col border-r border-[#1a202c]">
          <DocViewer
            ref={origRef}
            spans={result.original_spans}
            side="original"
            title={`ORIGINAL — ${result.file_a_name}`}
            zoom={zoom}
            currentChangeIndex={curChange}
            changeSpanIndices={changeSpanIndices}
        ...
        <div className="flex-1 min-w-0 flex flex-col border-r border-[#1a202c]">
          <DocViewer
            ref={revRef}
            spans={result.revised_spans}
            side="revised"
            title={`REVISED — ${result.file_b_name}`}
            zoom={zoom}
            currentChangeIndex={curChange}
            changeSpanIndices={changeSpanIndices}
